...
#' @examples
#' 
change_file_name <- function(file_path, metadata) {
      if(missing(metadata)) {
            cat("The path to the metadata reference file must be set")
            stop()
      } else {
      ## Getting track names
      track_names <- readxl::read_xlsx(metadata) %>% 
            janitor::clean_names() %>% 
            select(nombre_del_archivo_gps, numero_del_tracker) %>% 
            unique()
      ...
      write_csv(cleaned, paste0("preprocessed/", unique(cleaned$nombre_del_archivo_gps_corregido), ".csv"))
